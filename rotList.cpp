...
int main()
{
	...
	int rotations = 2;
	rotList(combinedList, rotations);
	std::cout << "\n\nThe order of the list after rotating it " << rotations << " times becomes:\n";
	for (std::list<int>::iterator tempIT = combinedList.begin(); tempIT != combinedList.end(); ++tempIT)
	{
		std::cout << " " << *tempIT;
	}
...
void rotList(std::list<int> &theList, int numRotations)
{
